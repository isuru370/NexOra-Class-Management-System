...
class CreateStudentCoursePaymentsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('student_course_payments', function (Blueprint $table) {
            $table->id('payment_id');
            $table->foreignId('registration_id')->constrained('student_registration')->onDelete('cascade');
            ...
            $table->string('month_year', 7)->nullable()->comment('Format: YYYY-MM for monthly payments');
            $table->integer('month_number')->nullable()->comment('Month number in the payment plan');
            $table->string('payment_method', 50)->default('cash')->comment('cash, card, bank_transfer, check');
